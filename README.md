...
With more computing power, one could also try to improve the arithmetic Asian hedging by replacing the geometric delta with a Monte-Carlo difference quotient approximation.  This last improvement could in fact be extended to geometric hedging as well, since the delta in that case is computed assuming continuous averaging, which is just an approximation of what happens in real markets, although how much doing so could actually improve hedging in a real-world environment is questionable.

To improve computing time beyond trying to vectorize code further, one should also explore stochastic PDEs methods to replace Monte-Carlo simulations, where possible.
